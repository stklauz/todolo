...
export default function useDragReorder(
  ...
  const isChildOf = React.useCallback(
    (sourceId: number, targetId: number): boolean => {
      ...
      debugLog('isChildOf: TRUE - target is child of source', {
        sourceId,
        targetId,
      });
      return true;
  ...
  const handleDragOver = React.useCallback(
    (event: React.DragEvent, targetId: number) => {
      ...
      if (targetSection !== currentDragInfo.section) {
        debugLog('Sections do not match', {
      ...
      if (isChildOf(currentDragInfo.id, targetId)) {
        debugLog('Cannot drop parent under child', {
          sourceId: currentDragInfo.id,
          targetId,
        });
        setDropTargetId(null);
        return;
  ...
  const handleDropOn = React.useCallback(
    (targetId: number) => {
      ...
      const targetSection = sectionOf(targetId);
      if (targetSection !== currentDragInfo.section) {
        debugLog('Sections do not match, ignoring drop', {
          sourceSection: currentDragInfo.section,
          targetSection,
        });
      ...
      if (isChildOf(sourceId, targetId)) {
        debugLog('Cannot drop parent under child, ignoring drop', {
          sourceId,
          targetId,
        });
        return handleDragEnd();
      ...
      debugLog('Processing normal drop', {
        sourceId,
        targetId,
        sourceSection: currentDragInfo.section,
        targetSection,
      ...
      setTodos((prev) => {
        ...
        if (computeSection(prev, sourceId) !== computeSection(prev, targetId))
          return prev;
        const next = [...prev];
  ...
  const handleDragOverEndZone = React.useCallback(
    (event: React.DragEvent, section: Section) => {
      ...
      if (!currentDragInfo || currentDragInfo.section !== section) {
        debugLog('Invalid end zone drag over', {
          section,
          dragInfo: currentDragInfo,
        });
        setDropAtSectionEnd(null);
  ...
  const handleDragLeaveEndZone = React.useCallback((section: Section) => {
    debugLog('Drag leave end zone', {
      section,
      currentDropAtSectionEnd: dropAtSectionEnd,
    });
    ...
    (section: Section) => {
      ...
      setTodos((prev) => {
        ...
        for (let i = next.length - 1; i >= 0; i--) {
          if (computeSection(next, next[i].id) === section) {
            lastIdx = i;
            break;
          }
        }
