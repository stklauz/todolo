import { loadListsIndex, saveListsIndex, loadListTodos, saveListTodos } from '../renderer/features/todos/api/storage';
...
describe('Storage API', () => {
  ...
  describe('loadListsIndex', () => {
    it('should return default data when no data exists', async () => {
      mockInvoke.mockResolvedValue({ version: 2, lists: [], selectedListId: undefined });
    ...
    it('should return loaded data when valid', async () => {
      const mockData = {
        version: 2,
        lists: [{ id: '1', name: 'Test List', createdAt: '2024-01-01T00:00:00.000Z' }],
        selectedListId: '1',
      };
  ...
  describe('saveListTodos', () => {
    it('should save todos successfully', async () => {
      ...
      expect(result).toBe(true);
      expect(mockInvoke).toHaveBeenCalledWith('save-list-todos', 'list-1', mockData);
    });
